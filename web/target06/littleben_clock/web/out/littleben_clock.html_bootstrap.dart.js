...
$$.ListQueue = {"": "Collection;_table,_liblib2$_head,_liblib2$_tail,_modificationCount",
  ...
  elementAt$1: function(_, index) {
    ...
    t1 = this._table;
    t2 = t1.length;
    t3 = (this._liblib2$_head + index & t2 - 1) >>> 0;
    if (t3 < 0 || t3 >= t2)
      throw $.ioore(t3);
    return t1[t3];
  },
    ...
    for (right = left, left0 = right, comp = null; true;) {
      ...
      if (comp > 0) {
        ...
        if (t1.get$left(current) == null)
          break;
        comp = $.compareTo$1$ns(t1.get$left(current).get$key(), key);
        if (comp > 0) {
        ...
        if (comp < 0) {
          ...
          if (t1.get$right(current) == null)
            break;
          comp = $.compareTo$1$ns(t1.get$right(current).get$key(), key);
          if (comp < 0) {
...
$.main = function() {
  var milliseconds, today;
  $.useObservers = false;
  milliseconds = $.Duration$(0, 0, 0, 0, 0, 1).get$inMilliseconds();
  if (milliseconds < 0)
    milliseconds = 0;
  $.TimerImpl$periodic(milliseconds, $.updateTime);
  today = $.DateTime$_now();
  $.currentTime = $.formatTime(today.get$hour(), today.get$minute(), today.get$second());
  $.dispatch();
  $.init_autogenerated();
